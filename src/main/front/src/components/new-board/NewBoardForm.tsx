...
export default function NewBoardForm() {
    ...
    const postBoardsMutation = useMutation((params: newBoardParams) => newBoardRequest(params, authCtx.authorization), {
        onSuccess: () => {
            // queryClient.invalidateQueries('persons');
            setIsLoading(false)
            dispatch(alertActions.setMessage("게시물 작성을 완료하였습니다!"));
    ...
    const handleSubmit = async () => {
        ...
        setIsLoading(true)
        postBoardsMutation.mutate(params);
    }
    ...
    const undoCheck = () => {
        dispatch(warningActions.setMessage("게시물 작성을 취소합니다"));
    ...
    const submitCheck = (event: React.FormEvent) => {
        ...
        dispatch(warningActions.setMessage("게시물을 작성하시겠습니까?"));
        dispatch(warningActions.open());
    ...
    return (
        <Form noValidate onSubmit={submitCheck}>
            ...
            <Form.Group className="d-flex justify-content-center mb-3" controlId="exampleForm.ControlTextarea1">
                ...
                <Button className={"me-3"} type="submit" disabled={isLoading}>등록</Button>
            </Form.Group>
        </Form>
    )
}
